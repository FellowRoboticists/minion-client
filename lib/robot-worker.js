...
module.exports = (function () {
  ...
  const RobotWorker = function (robotName, serverKey, robot, robotCfg) {
    winston.log('info', robotCfg)
    this.robotName = robotName
    this.serverKey = serverKey
    this.robot = robot
    this.robotCfg = robotCfg
    this.defaultSpeed = robotCfg.defaultSpeed
    this.speed = robotCfg.defaultSpeed
    ...
    this.__drive = function (speed, direction) {
      ...
      if (this.robotCfg.hasSafeCommand) {
    ...
    this.process = function (msg) {
      winston.log('debug', 'Starting to process reserved job')
      return signer.verify(msg, this.serverKey)
        .then((payload) => {
